...
class IpData(GenericBaseModel[IPGeolocationResponse, None]):
    ...
    def getLocation(self) -> Optional[LocationInfo]:
        if self.info and self.info.latitude and self.info.longitude:
            return LocationInfo(
                latitude=float(self.info.latitude), longitude=float(self.info.longitude)
            )
        return None
            ...
            checks.extend(
                [
                    WarningStatus(
                        ...
                        category="vpn",
                        message=(
                            "⚠️ VPN usage detected in server"
                            if self.isServerVpn()
                            else "✅ No VPN detected in server"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="proxy",
                        message=(
                            "⚠️ Proxy usage detected in server"
                            if self.isServerProxy()
                            else "✅ No proxy detected in server"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="tor",
                        message=(
                            "⚠️ Tor usage detected in server"
                            if self.isServerTor()
                            else "✅ No Tor detected in server"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="relay",
                        message=(
                            "⚠️ Relay usage detected in server"
                            if self.isServerRelay()
                            else "✅ No Relay detected in server"
                        ),
                    ),
                ]
            )
            ...
            checks.extend(
                [
                    WarningStatus(
                        ...
                        category="vpn",
                        message=(
                            "⚠️ VPN usage detected in header"
                            if self.isHeaderVpn()
                            else "✅ No VPN detected in header"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="proxy",
                        message=(
                            "⚠️ Proxy usage detected in header"
                            if self.isHeaderProxy()
                            else "✅ No proxy detected in header"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="tor",
                        message=(
                            "⚠️ Tor usage detected in header"
                            if self.isHeaderTor()
                            else "✅ No Tor detected in header"
                        ),
                    ),
                    WarningStatus(
                        ...
                        category="relay",
                        message=(
                            "⚠️ Relay usage detected in header"
                            if self.isHeaderRelay()
                            else "✅ No Relay detected in header"
                        ),
                    ),
                ]
            )
    ...
    def get_ip_mismatch(self) -> WarningStatus:
        ...
        return WarningStatus(
            ...
            message=(
                "⚠️ IP Address Mismatch: Server IP differs from Header IP"
                if header_ip_changed
                else "✅ IP addresses match"
            ),
        )
    ...
    def get_country_mismatch(self) -> WarningStatus:
        ...
        server_country = (
            self.server.country_code2 if self.server and self.server.country_code2 else None
        )
        header_country = (
            self.header.country_code2 if self.header and self.header.country_code2 else None
        )
        ...
        return WarningStatus(
            ...
            message=(
                "⚠️ Country Mismatch: Server country differs from Header country"
                if country_mismatch
    ...
    def get_timezone_mismatch(self) -> WarningStatus:
        ...
        return WarningStatus(
            ...
            message=(
                "⚠️ Timezone Mismatch: Header timezone differs from IP timezone"
    ...
    def get_locale_mismatch(self) -> WarningStatus:
        ...
        server_locale = (
            self.server.languages.split(",")[0] if self.server and self.server.languages else None
        )
        header_locale = (
            self.header.languages.split(",")[0] if self.header and self.header.languages else None
        )
        ...
        return WarningStatus(
            ...
            message=(
                "⚠️ Locale Mismatch: Server locale differs from Browser locale"
                if locale_mismatch
                else "✅ Locales match"
            ),
        )
...
class UserAgentData(GenericBaseModel[str, UserStackResponse]):
    ...
    def get_user_agent_mismatch(self) -> WarningStatus:
        ...
        return WarningStatus(
            ...
            message=(
                "⚠️ User Agent Mismatch: Server and Client user agents differ"
                if self.has_user_agent_mismatch()
                else "✅ User agents match"
            ),
        )
